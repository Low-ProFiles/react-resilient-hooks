...
function useBackgroundSync(options = {}) {
  ...
  const log = react.useMemo(() => createLogger(debug), [debug]);
  ...
  const flushLockRef = react.useRef(null);
  const mountedRef = react.useRef(true);
  ...
  const enqueue = react.useCallback(async (url, fetchOptions, meta) => {
  ...
  const processRequest = react.useCallback(async (req, signal) => {
  ...
  const abortFlush = react.useCallback(() => {
    if (abortControllerRef.current) {
      log("Aborting flush");
      abortControllerRef.current.abort();
    }
  }, [log]);
  const getQueueSize = react.useCallback(async () => {
...
exports.RETRYABLE_STATUS_CODES = RETRYABLE_STATUS_CODES;
...
exports.isNetworkInformationSupported = isNetworkInformationSupported;
